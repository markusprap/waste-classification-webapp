...
export async function POST(request) {
  try {
    ...
    if (!session || !session.user) {
      console.log('Guest user detected, allowing limited classification');
      user = {
        id: 'guest',
        email: 'guest@wastewise.ai',
        name: 'Guest User',
        plan: 'guest',
        usageCount: 0,
        usageLimit: 5
    ...
    if (user.id !== 'guest') {
      ...
      console.log('Guest classification - skipping DB persistence and usage sync');
    }
    ...
    return NextResponse.json({
      ...
      usage: {
        ...
        limit: user.plan === 'free' ? 30 : user.plan === 'premium' ? 10000 : user.plan === 'guest' ? 5 : 'unlimited',
